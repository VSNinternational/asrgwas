...
#'
#' @description
#' See \link[ASRgblup]{audit.gblup} and \link[ASRgwas]{audit.gwas}
#' @keywords internal
audit.generic <- function(
  ...
  if (is.matrix(Kinv)) Kinv <- list(Kinv)
  ifelse(test = is.null(geno.data), yes = {geno.data.type <- "NIL"}, no = {geno.data.type <- "OBJ"})
  match.data(
    pheno.data = pheno.data, indiv = indiv,
    geno.data = switch(geno.data.type, NIL = NULL, OBJ = list(geno.data)),
    Kinv = Kinv, Q = Q, map.data = map.data, caller = caller, message = message)
  if (!is.null(geno.data)){
    miss.geno.data <- sum(is.na(geno.data))/length(geno.data) * 100
    if (message & miss.geno.data > 0) {
      message("Missing data found in `geno.data` (", round(miss.geno.data, 2), "%).")
    }
    rm(miss.geno.data)
  }
  if (!is.null(resp)) {
  ...
  if (message) {message(paste0("A total of ", out$n.genotypes, " unique genotypes were found."))}
  if (!is.null(geno.data)){
    out$n.markers <- ncol(geno.data)
    if (message) {message(paste0("A total of ", out$n.markers, " markers were found."))}
  }
  if (!is.null(geno.data)){
    out$ind.callrate.stats <- ASRgenomics:::callrate(M = geno.data, margin = "row")
    out$ind.callrate.stats <- round(range(out$ind.callrate.stats), 2)
    if (message){
      message(paste0("Genotype call rate range in `geno.data` is: ",
                     paste0(out$ind.callrate.stats, collapse = " ~ "), "."))
    }
    out$marker.callrate.stats <- ASRgenomics:::callrate(M = geno.data, margin = "col")
    out$marker.callrate.stats <- round(range(out$marker.callrate.stats), 2)
    if (message){
      message(paste0("Marker call rate range in `geno.data` is: ",
    ...
    if (message){
      message(paste0("Minor allele frequency in `geno.data` is: ",
  ...
  if(!is.null(Kinv)){
    ...
    else names_of_Kinv <- names(Kinv)
  }
  if(!is.null(Kinv)){
    out$Kinv.condition <- lapply(Kinv, ASRgenomics:::Kinv.condition)
    out$Kinv.condition <- unlist(out$Kinv.condition)
    if (message){
      message(paste0("Condition of `Kinv` matrix(ces) is:",
                     paste("\n ", names_of_Kinv, ": ",
                           out$Kinv.condition, sep = "", collapse = ", "), "."))
    }
  }
  return(out)
}
